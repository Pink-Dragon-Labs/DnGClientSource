...
;;	Big/Little Endian
		(= pServIDLo (aPacket getWord:))
		(= pServIDHi (aPacket getWord:))

		(if (not (self hasBase: BCharacter))
			(= pColor (aPacket getWord:))
		)
		...
		(if (aPacket getByte:)
			...
			(KArray ArrayFree (tStr data?))
		 	(tStr data: (aPacket getString: 0))
			(self setName: tStr)
		)

		(while (!= (= tValue (aPacket getByte:)) 255)
			(self setAffectedState: tValue)
		)

		;; check to see if this object is money
		(switch (aPacket getByte:)
			(1
				(= pMoney TRUE)
				(= tValue (LongInt new:))
				(tValue set: (aPacket getWord:) 0)
			)
			
			(2
				(= pMoney TRUE)
				(= tValue (LongInt new:))
				(aPacket getLong: tValue)
			)
		)
		...
		(if (= tBase (self getBase: BCharacter))
			(= gEgoStepX (aPacket getByte:))
			(= gEgoStepY (aPacket getByte:))

			(self setStep: gEgoStepX gEgoStepY)

			(= xScaleAdj (aPacket getByte:))
			(= yScaleAdj (aPacket getByte:))
		...
		(if tRoomType
			...
			(switch tRoomType
				(1
					(tLong set: (aPacket getWord:) 0)
				)
				(2
					(aPacket getLong: tLong)
		 		)
			)
