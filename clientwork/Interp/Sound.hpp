...
// this structure defines the avaialble settings for a digital sound driver
typedef struct {
	int m_nRate, m_nBits, m_nChannels;
	bool m_bDirectX;
} digitalsnd_t;
...
public:
	...
	int getSfxMasterVolume ();

	// set the volume for SFX
	int setSfxMasterVolume ( int volume );

	// get the volume for MIDI
	int getMidiMasterVolume ();

	// set the volume for MIDI
	int setMidiMasterVolume ( int volume );
