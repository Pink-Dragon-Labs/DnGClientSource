...
const int nEntries = sizeof(entries) / sizeof(*entries);
...
// BEW CLEANUP 	printf("INTERPRETER VERSION : %s   ", version);
// BEW CLEANUP 		printf("VERSION STAMP: %d   ", GetVerStamp());
	exit(1);
}
...
ConfigMgr::Tokenize(char* buf)
{
	...
	for (nTokens = 0; tmp = strtok(0, " =\t;,"); nTokens++) {
		strcpy(buf, tmp);
		buf += strlen(tmp) + 1;
	}
