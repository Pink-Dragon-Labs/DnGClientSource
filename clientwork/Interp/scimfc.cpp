...
BOOL CSciApp::InitInstance ( void )
{
	...
	if ( RegOpenKeyEx ( HKEY_LOCAL_MACHINE, "Software\\Microsoft\\Internet Explorer", 0, KEY_QUERY_VALUE, &hRegKey ) == ERROR_SUCCESS ) {
	...
	if ( nResult != 0 ) {
		msgMgr->Fatal ( "NETERR: The WINSOCK.DLL that you have is not suitable.  You need a version 1.1 WINSOCK.DLL to use this application.  You have version (0x%x, 0x%x) of the library.  Result code is %d!", wsaData.wHighVersion, wsaData.wVersion, nResult );
	}
	...
	if ( nColors < 16 ) {
		if ( IDYES == ::MessageBox ( ::GetFocus(), "Your desktop is currently set to support less than 16 bit color.  The Realm Online requires that your desktop support at least 16 bit color.  Would you like to change your desktop settings and continue playing?", "Unsupported Color Depth", MB_TASKMODAL | MB_YESNO ) ) {
			...
			if ( nResult == 0 ) {
				::MessageBox ( ::GetFocus(), "Your desktop has been reconfigured.  Click OK to continue playing.", "Success", MB_TASKMODAL );
			}

			else if ( nResult == 1 ) {
				::MessageBox ( ::GetFocus(), "Your desktop has been reconfigured, but you must restart Windows for the new settings to take effect.  Please restart Windows.", "Restart Required", MB_TASKMODAL );
				exit ( 1 );
			}

			else {
				::MessageBox ( ::GetFocus(), "Your desktop could not be reconfigured.  Please check your system settings and try again.", "Error", MB_TASKMODAL );
				exit ( 1 );
			}
		} else {
		 	exit ( 1 );
				...
				if ( m_msgCur.wParam == VK_RETURN ) {
					...
					if ( m_msgCur.lParam & 0x20000000 ) 
                    {
						if ( g_pSpellListDialog == NULL &&  g_pWindowMgr && ( g_pWindowMgr->GetGameModal(gmCombatModal) == 0 ) && g_pMainPanelWnd && g_pMainPanelWnd->Spells() )
                        {
							...
							(new CSpellListDlg( 0, 0 ))->Create( m_pMainWnd );
                        }
